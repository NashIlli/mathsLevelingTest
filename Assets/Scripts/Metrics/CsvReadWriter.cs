    ...
    public class CsvReadWriter
    {
        ...
        public void CreateNewCSV(int level)
        {
            ...
            if (File.Exists(path))
            {
                ...
                for (int i = 0; i < MathsController.GetController().GetTotalActivities(); i++)
                {
                    string activity = MathsController.GetController().GetActivityName(i);
                    rowDataTemp.Add(activity +  " Correct");
                    rowDataTemp.Add(activity + " Hints");
                }                         
