...
namespace Assets.Scripts.Metrics
{
    ...
    public class MetricsController : MonoBehaviour
    {
        ...
        private void SaveToDisk()
        {
            ...
            FileStream file = File.Create(Application.persistentDataPath + "/" + (AppController.GetController().GetCurrentChallenge()+1) + "level.dat");
        ...
        {
            if (File.Exists(Application.persistentDataPath + "/" + (AppController.GetController().GetCurrentChallenge() + 1) + "level.dat"))
            {
                BinaryFormatter bf = new BinaryFormatter();
                FileStream file = File.Open(Application.persistentDataPath + "/" + (AppController.GetController().GetCurrentChallenge() + 1) + "level.dat", FileMode.Open);
